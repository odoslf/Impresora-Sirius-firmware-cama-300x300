...
def set_define(text, name, value=None, required=True):
    ...
    pattern = re.compile(rf"(?m)^\s*(?://\s*)?#define\s+{re.escape(name)}(?:[ \t]+[^\n]*)?$")
...
def comment_define(text, name, required=False):
    ...
    if n == 0 and required:
        raise RuntimeError(f"No se encontró define activo {name}")
    return new

cfg = CFG.read_text(encoding="utf-8")
adv = ADV.read_text(encoding="utf-8")

# IDENTIDAD / PLACA / COMUNICACION
...
# DRIVERS TMC2209 STANDALONE. X2 se autoasigna al primer E libre (E2 en RUMBA con E0/E1 ocupados).
...
# ENDSTOPS: X1 XMIN, X2 XMAX, Y YMIN, Z ZMIN. NC con pullups.
...
# ADVANCED: protecciones, ventiladores, IDEX y homing
...
checks = {
    CFG: [
        "#define MOTHERBOARD BOARD_RUMBA", "#define BAUDRATE 115200",
        "#define X_DRIVER_TYPE TMC2209_STANDALONE", "#define X2_DRIVER_TYPE TMC2209_STANDALONE",
        "#define Y_DRIVER_TYPE TMC2209_STANDALONE", "#define Z_DRIVER_TYPE TMC2209_STANDALONE",
        "#define E0_DRIVER_TYPE TMC2209_STANDALONE", "#define E1_DRIVER_TYPE TMC2209_STANDALONE",
        "#define EXTRUDERS 2", "#define TEMP_SENSOR_0 5", "#define TEMP_SENSOR_1 5", "#define TEMP_SENSOR_BED 1",
        "#define X_BED_SIZE 295", "#define Y_BED_SIZE 295", "#define X_MIN_POS -48", "#define Y_MIN_POS -12",
        "#define X_MAX_POS 295", "#define Y_MAX_POS 295", "#define Z_MAX_POS 275",
        "#define DEFAULT_AXIS_STEPS_PER_UNIT { 80.19, 80.19, 400, 96 }",
        "#define THERMAL_PROTECTION_HOTENDS", "#define THERMAL_PROTECTION_BED"
    ],
    ADV: [
        "#define DUAL_X_CARRIAGE", "#define X2_MIN_POS 25", "#define X2_MAX_POS 359",
        "#define X2_HOME_POS X2_MAX_POS", "#define DEFAULT_DUPLICATION_X_OFFSET 150",
        "#define E0_AUTO_FAN_PIN 6", "#define E1_AUTO_FAN_PIN 8"
    ]
}
...
print("Migración de configuración Sirius aplicada correctamente")
